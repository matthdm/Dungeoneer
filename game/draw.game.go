...
func (g *Game) drawPlaying(screen *ebiten.Image, cx, cy float64) {
	...
	if g.player != nil && len(g.cachedTriangles) > 0 && !g.FullBright {
		fov.DrawShadowMask(screen, g.cachedTriangles, g.camX, g.camY, g.camScale, cx, cy, g.currentLevel.TileSize)
	}
