...
>
> Void Rift VFX shader pilot — Kage-shader glow/rift primitives proving out a new visual pipeline on two new CR1 void-domain artifacts (`void_rift_catalyst`, `voidbound_pendant`). Lives outside `plans/` (written under `docs/superpowers/`) but is tracked here as the actual in-flight work — do not treat it as untracked ad-hoc work. **Status as of 2026-09-15: Tasks 1–6/7 code-complete** (shader primitives, artifact registration + engine tests, both visuals, game-layer wiring, icons/flavor — all present and uncommitted; `go build ./...` clean, `go test ./combat/... ./items/... ./game/...` all `ok`). Only Task 7 Step 5 remains — manual in-game verification via the new "Void Rift Test" dev-menu entry, plus screenshot-based visual tuning per the design spec's Open Question #2 — deferred to a later session. Design spec: `docs/superpowers/specs/2026-07-26-void-rift-vfx-pilot-design.md`, plan: `docs/superpowers/plans/2026-07-26-void-rift-vfx-pilot.md`.
>
