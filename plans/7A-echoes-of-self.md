...
- [ ] After every run (death or victory), an `EchoRecord` is written to `echoes/run_N.json` capturing: floor-by-floor position snapshots (sampled every 2 seconds), equipped item IDs at death, cause of death string, floor of death.
- [ ] On floor generation, the echo spawner reads stored `EchoRecord` files and spawns 0–3 echo entities near recorded death locations on matching floors.
- [ ] `WickedEcho` behaves as a melee enemy with the original run's stats; drops 5–15 Remnants on defeat.
- [ ] `HeroEcho` follows the player and attacks nearby enemies for 60 seconds, then dissolves.
- [ ] `MemoryFragment` is a static ghost NPC; interaction shows a 1–2 line contextual message ("I fell here. Watch the shadows.").
- [ ] All echo entities render with a blue/purple tint and reduced opacity (alpha 0.6, flicker ±0.1 at 2Hz).
- [ ] Echo Shrine in hub (when unlocked) lists stored echo runs with date, floor of death, and cause; player can banish (delete) individual echo records.
- [ ] `cd src && go build ./...` passes.

## Phases

### Phase 1: Echo recorder and data model
- [ ] 1.1 Create `src/game/echo_recorder.go`. `EchoRecorder` struct with `Snapshots []PositionSnapshot`, `EquippedItems []string`, `DeathCause string`, `DeathFloor int`.
- [ ] 1.2 `PositionSnapshot`: `Floor int`, `X, Y float64`, `HP int`, `Timestamp float64`.
- [ ] 1.3 `Record(dt float64)` called each update tick; appends a snapshot every 2 seconds.
- [ ] 1.4 `Finalize(cause string, floor int)` called on run end; serializes to `echoes/run_N.json` where N is `MetaSave.CompletedRuns`.
- [ ] 1.5 In `src/game/metasave.go`, add `EchoFiles []string` to track echo record paths.
- [ ] 1.6 `cd src && go build ./...` passes.

### Phase 2: Echo entity types
- [ ] 2.1 Create `src/entities/echo.go`. Define `EchoType` enum and base `Echo` struct (position, type, source record reference).
- [ ] 2.2 `WickedEcho`: implements monster interface, melee behavior, HP scaled from source run's floor, drops Remnants on death.
- [ ] 2.3 `HeroEcho`: follows player, attacks nearest enemy in range, 60-second lifetime, no drops.
- [ ] 2.4 `MemoryFragment`: static, interaction triggers a short dialogue panel with a 1–2 line contextual message drawn from the death cause string.
- [ ] 2.5 `cd src && go build ./...` passes.

### Phase 3: Ghost visual style
- [ ] 3.1 In `src/game/draw.game.go`, detect echo entities in the draw list. Apply `ColorScale` to set RGB to blue/purple tint (R: 0.4, G: 0.5, B: 1.0) and alpha to 0.6.
- [ ] 3.2 Flicker: use a sine wave on game tick to oscillate alpha between 0.5 and 0.7 at 2Hz.
- [ ] 3.3 `cd src && go build ./...` passes.

### Phase 4: Echo spawner and shrine
- [ ] 4.1 In `src/game/hub.go` floor generation, after room layout: load echo records, for each record where `DeathFloor == currentFloor`, pick 1 echo type (weighted: 50% Wicked, 30% Hero, 20% Memory), spawn near the death position. Cap at 3 echoes per floor.
- [ ] 4.2 In `src/game/hub.go` hub load, spawn Echo Shrine entity when `HubState["echo_shrine_unlocked"]`.
- [ ] 4.3 Create `src/ui/echo_shrine.go`. List stored echo runs (date, floor, cause). Button to banish (delete file and remove from `MetaSave.EchoFiles`). Close button.
