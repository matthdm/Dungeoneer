...
### Mana routing
~~The engine currently does not model mana costs.~~ **Done (2026-07-05):** `ArtifactEffect.ManaCost` set on all 9 spells (values match the legacy `spellManaCost` table). The engine enforces and deducts mana on activation and emits `EventManaChanged`; the adapter mirrors the spend to `g.player.Mana`. Enforcement is gated on `PlayerMaxMana > 0` so benchmarker scenarios (which don't populate mana) are unaffected — parity re-verified against the CR1-E table. `ManaCostReductionPct` carries item mana-cost reduction (and the `InfMana` dev flag as 100%). hollow_sigil void skills skip mana and pay HP as before.
...
| 2026-07-06 | Bespoke artifact visuals | ✅ Complete | Playtest feedback (round 2): every artifact skill (ironbreaker_slam, shadowstep, warden_taunt, void_bind, execute, arcane_surge_blast, blood_price_strike) was falling through to the generic domain-colored particle-burst fallback — no unique visual identity, contrary to each skill's description. Fixed by writing 7 bespoke visual effects in new `src/spells/artifacts.spells.go` (`GroundSlam`, `ShadowStrike`, `TauntPulse`, `RootBind`, `ReaperExecute`, `ArcaneSurgeNuke`, `BloodPriceStrike`), following the existing SlashArc/BlinkEffect/FractalCanopy idiom (vector.StrokeLine/DrawFilledCircle/StrokeCircle, phased age/Duration animation, IsFinished), each shaped to its mechanic (slam = cracked-earth shockwave ring, shadowstep = shadow tendrils + crit-flash on arrival, taunt = golden aggro pulse + spinning shield glyph, root = chains snapping shut, execute = scythe arc + skull flash on kill, surge = converging bolts + detonation, blood price = HP-cost mist + void lance). Wired into `spawnSkillVisual`'s switch; `blinkBehindTarget` now spawns `ShadowStrike` instead of the plain `BlinkEffect` (kept distinct from the "blink" dash ability). `spawnSkillVisual` gained a `killed bool` param so the execute's skull-flash only fires on an actual kill — sourced from `a.combatState.TargetIsDead` (the post-tick engine state), not `g.TargetedMonster` (which is still the pre-death snapshot when `EventSkillFired` is processed, since the engine emits it before `EventDamageDealt`). **Also fixed:** arcane_spray was firing as a single discrete burst instead of its held channeled cone — root cause was routing it through the engine's one-press `ActionActivateSkill` model, which doesn't fit a hold-to-channel spell. `HandleSkillActivation` now special-cases `arcane_spray` to bypass the engine entirely and call the legacy `castSpellSlot`/`tryCastArcaneSpray`/`updateChanneledSpray` path (which runs unconditionally regardless of combat adapter) on both combat paths, restoring original behavior exactly. Pure-passive spell-bar entries (stone_skin_idol, soul_harvest, etc. — Cooldown 0 + IsPassive) no longer queue an action or fire a wasted particle burst on keypress. Build/vet/full test suite clean (same 2 pre-existing `levels` failures, unrelated); benchmarker re-verified exact parity on Iron Flurry, Perma Shadow, The 55, Arcane Farmer. |
| 2026-07-06 | Playtest fixes | ✅ Complete | Playtest revealed the 2026-07-05 adapter treated the MetaSave loadout as the authoritative build — but the real loadout was near-empty (`["","","","","","","grave_reaper"]`), so hotkeys were dead, passives (ember burn etc.) invisible, and no spell visuals fired. **Reversed the source of truth:** engine actives 0-5 now mirror the spell bar (ability IDs resolved to engine item IDs, identity slot mapping); slot 6 = loadout elite; new `CombatState.PassiveArtifacts` carries worn equipment with passive engine effects (engine scans both via `scanArtifacts`; sims unaffected — parity re-verified exact). **Build persistence:** `Player.LoadoutAbilities` records loadout-granted abilities; `RefreshAbilities` re-applies them after rebuilding from equipment (previously any equip wiped the loadout build); cleared in `ClearAbilities` and re-recorded by `equipArtifactLoadout`. **Targeting feel:** click-select radius widened 0.6 → 1.3 tiles (nearest-within wins) — clicks land on the ground plane while sprites are tall. 2 new engine tests (passive-channel burn + heal-on-kill). Known minor: engine cooldowns are keyed by slot index, so re-ordering the bar mid-cooldown migrates the remaining cooldown to whatever lands in that slot. |
| 2026-07-05 | CR1-D/E gap fill | ✅ Complete | **Visual bridge:** `EventSkillFired` now spawns the real legacy spell visuals in visual-only mode (`VisualOnly` flags on Fireball/ArcaneBolt/LightningStorm/FractalBloom; `DamageApplied` pre-set on strikes/nodes) via new `src/game/spell_visuals.go`; particle burst remains the fallback for artifact skills without bespoke visuals. Blink strike (shroud_cloak) now teleports the player behind the target via `blinkBehindTarget` + BlinkEffect trail. Canopy still heals through the game layer (engine doesn't model healing fields). **Mana:** engine-enforced, see Mana routing section. **Adapter fixes:** (1) engine now ticks every frame — cooldowns, shadow/root/burn timers, and streaks no longer freeze when out of combat or out of range; melee skills gained an in-range gate in the engine, blink exempt as a gap-closer; (2) ID canonicalization — SpellSlots hold ability IDs (`ironbreaker_slam`) but `ArtifactEffects` is keyed by item IDs (`ironbreaker_gauntlets`); real (non-dev) runs previously fell back to stub cooldowns and **passive stat items in the loadout were invisible to the engine**. `EquippedArtifacts` is now built from the MetaSave loadout (canonicalized), with a spell-bar→engine-slot translation table for activation and HUD; (3) casting with no target auto-acquires the monster nearest the cursor (4-tile radius); a truly targetless cast burns cooldown+mana and shows the visual but can't ghost-kill (engine guards added); (4) HUD cooldown display reads engine cooldowns via `BarCooldown()`. 7 new engine tests in `mana_test.go`; all combat tests + benchmarker parity (Iron Flurry, Arcane Farmer, Perma Shadow, The 55 exact-match the CR1-E table) verified. Remaining known gaps: arcane_spray has no channeled visual on the new path (burst fallback); artifact icons are still procedural placeholders (needs art). |
